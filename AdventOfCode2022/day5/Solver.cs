    ...
    internal class Solver : Problem
    {
        public override T Solve<T>(ProblemChoice pc)
        {
            ...
            foreach (string order in orders)
            {
                ...
                List<string> cratesToMove = innerCrate.TakeLast(quantity).ToList();
                if (pc == ProblemChoice.B) cratesToMove.Reverse();

                for (int i = 0;  i < quantity; i++)
                {
                    var item = cratesToMove[cratesToMove.Count - 1];
                    cratesToMove.RemoveAt(cratesToMove.Count - 1);
